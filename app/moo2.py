"""NSGA-II powered optimisation endpoints."""
...
def moo2_rocket(req: RocketRequest) -> dict[str, object]:
    rng = random.Random(req.seed)
    initial = r_sample_lhs(req.samples, seed=req.seed)

    def objective(design: dict) -> tuple[float, float, float]:
...
def moo2_pencil(req: PencilRequest) -> dict[str, object]:
    rng = random.Random(req.seed)
    ...
    metrics = [m for m in metrics if m["ok"]]
    metrics.sort(key=lambda m: (-m["spec_thrust_N_per_kgps"], m["TSFC_kg_per_Ns"], m["f_fuel"]))
    return {"top": metrics[: req.topk]}
