...
def text(x,y,t,fs=12,fill="#fff",anc="start"): return f"<text x='{x}' y='{y}' fill='{fill}' font-size='{fs}' text-anchor='{anc}'>{t}</text>"
...
def saturn_overview_h(scale: float = Query(10.0, ge=4.0, le=24.0)):
    ...
    def stage(x, L, D, title, eng_notes):
        ...
        # 내부(간단): 상부 LOX, 인터(또는 벌크헤드), 하부 LH2/RP-1
    ...
    x  = stage(x, L1, D33, "S-IC (1단, Ø33 ft)",  "F-1 ×5")
    x +=  gap
    x  = stage(x, L2, D33, "S-II (2단, Ø33 ft)",  "J-2 ×5")
    x +=  gap
