...
def tri(cx,cy,w,h,stroke="#9cd0ff",fill="#1f6aa8",sw=1):
    ...
    return f"<path d='M {x1} {y1} L {x2} {y2} L {x3} {y3} Z' fill='{fill}' stroke='{stroke}' stroke-width='{sw}'/>"
...
def saturn_overview_h(scale: float = Query(12.0, ge=4.0, le=24.0)):
    try:
        ...
        pad_top   = 70
        gap       = 110         # 단 간격 조금 더
        foot_pad  = 140         # 하단 여백 넉넉히
        maxL      = max(L1,L2,L3)
        w33 = D33*mm; w21 = D21*mm
        W   = int(pad_left*2 + w33 + gap + w33 + gap + w21 + 280)
        ...
        def stage(x, L, w, title_top, title_bot, engine_note, is_s4b=False):
            h = L*mm
            top = baseY - h
            svg.append(box(x, top, w, h, stroke="#fff", sw=1.6))
            if not is_s4b and "S-IC" in title_top:
                lox=L*0.42; inter=0.9; rp1=L-lox-inter
                svg.extend([ tank_capsule(x+6, top+6, w-12, lox*mm-6, 26, "#9fd3ff"),
            ...
            elif not is_s4b and "S-II" in title_top:
                lox=L*0.22; lh2=L*0.73
                svg.extend([ tank_capsule(x+6, top+6, w-12, lox*mm-6, 22, "#9fd3ff"),
                ...
                svg.extend([ tank_capsule(x+6, top+6, w-12, lox*mm-6, 18, "#9fd3ff"),
            ...
            # 아래 2줄 라벨(겹침 방지)
            svg.append(text(x+w/2, baseY+40, engine_note, 11, "#9cd0ff", "middle"))
            svg.append(text(x+w/2, baseY+58, f"{title_top}", 12, "#9fd3ff", "middle"))
            svg.append(text(x+w/2, baseY+72, f"{title_bot}", 11, "#9fd3ff", "middle"))
            return x+w

        x = pad_left
        x  = stage(x, L1, w33, "S-IC (1단)",  "Ø33 ft",  "F-1 ×5")
        x += gap
        x  = stage(x, L2, w33, "S-II (2단)",  "Ø33 ft",  "J-2 ×5")
        x += gap
        x  = stage(x, L3, w21, "S-IVB (3단)", "Ø21.7 ft","J-2 ×1", is_s4b=True)
