"""Generate a simple aerospike-like nozzle profile for demos."""
...
def nozzle_profile(rt: float, eps: float, spike_deg: float, n: int = 100) -> List[Tuple[float, float]]:
    """Return a 2D profile ``(x, r)`` for the spike body."""

    spike_deg = max(spike_deg, 1.0)
    area_t = math.pi * rt * rt
    re = math.sqrt(eps * area_t / math.pi)
    length = (re - rt) / math.tan(math.radians(spike_deg))

    profile = []
    for i in range(n + 1):
        frac = i / n
        x = length * frac
        radius = max(rt + (re - rt) * frac ** 0.8, 1e-6)
...
def revolve_to_triangles(profile: List[Tuple[float, float]], seg: int = 64):
    """Revolve the profile around the x-axis and emit STL triangles."""
...
__all__ = ["nozzle_profile", "revolve_to_triangles"]
