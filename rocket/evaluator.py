...
def _manufacturing_rules() -> tuple[float, float, float]:
    path = Path("manufacturing/rules.json")
    if path.is_file():
        try:
            data = json.loads(path.read_text(encoding="utf-8"))
            rocket_cfg = data.get("rocket", {})
            return (
                float(rocket_cfg.get("min_wall_mm", 1.2)),
                float(rocket_cfg.get("min_web_mm", 0.8)),
                float(rocket_cfg.get("r_mid_scale", 1.4)),
            )
        except Exception:
            return 1.2, 0.8, 1.4
    return 1.2, 0.8, 1.4
...
def evaluate_batch(designs: List[Dict[str, float]], pa_kpa: float = 101.325) -> List[Dict[str, float]]:
    ...
    for design in designs:
        ...
        r_mid = r_mid_scale * throat_radius
        pitch_m = (2.0 * math.pi * r_mid) / max(channel_count, 1)
        web_thickness_mm = max(pitch_m - channel_diameter, 0.0) * 1e3
        mfg_ok = (web_thickness_mm >= min_web_mm) and (min_wall_mm >= 1.0)
        ...
        results.append(
            {
                ...
                "web_thickness_mm": float(web_thickness_mm),
                "mfg_ok": bool(mfg_ok),
                "design": design,
            }
        )
